...
export default function Product({ serchProductid}) {
  ...
  useEffect(() => {
    ...
    if(product && product.messages){
      ...
      let fiterMessageArray=product.messages.filter(msg=>{
        return msg.buyer['_id']===loginUser['_id']
      })
      // console.log("fiterMessageArray",fiterMessageArray)
  ...
  const fetchSingleProductData = async () => {
    ...
    // console.log("sellerresponse.data",sellerresponse.data)
      ...
      {(product && seller) &&
        <div className="container-wrap">
          <div className='product'>
                ...
                <div className='product_container product-data-wrap'>
            ...
            <div className='product_container seller-data-wrap'>
                 <div className='seller-info'>
                      ...
                      <div className='seller_info'>
                        ...
                        <div>
                          <p><b>Address:-</b>{`${product.address['area']}, city-${product.address['city']}, state;-${product.address['state']}`}</p>
                        </div>
                        <div>
                          <p><b>Pin-Code:-</b>{product.address['zipCode']}</p>
                        </div>
                ...
                <div className='chatroom-wrapper'>
                          ...
                          <div className='chat-messages'>
                              <ul className='message-list' ref={messageListRef}>
                                  ...
                                  {messageArray.length?messageArray[0].chat.map(msg=>{
                                      return (
                                        <li className={msg[loginUser.name]?'sender':'reciver'}>
                    ...
                    <div className='chat-form'>
                      ...
                      <button type='submit' className='submit-btn' onClick={(e)=>{
                        const buyer=({"_id":loginUser["_id"],"name":loginUser.name})
                        sendMessage(e,newMsg,setNewMsg,buyer,seller,params.id,messageListRef,setFlag,"buyer")
                      }}>send</button>
                      </form>
                    </div>
